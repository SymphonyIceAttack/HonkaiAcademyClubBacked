...
Register.post("/", async (ctx) => {
    ...
    const RegisterParams = JSON.parse(json);
    ...
    isValidate &&
        (await saveImageFile(RegisterParams.avatarImage, "avatarImage", RegisterParams.account));
    isValidate &&
        (await saveImageFile(RegisterParams.equipmentImage, "equipmentImage", RegisterParams.account));
    isValidate &&
        (await saveImageFile(RegisterParams.MoeChapterImage, "MoeChapterImage", RegisterParams.account));
    isValidate
        ? (ctx.body = {
            msg: "注册成功",
            status: 200,
